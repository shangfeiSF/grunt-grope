...
// 'use strict';
function testXiaoShao (argA, argB){
  var t = 123;
  var t = 890;
  console.log(t);
  var obj = {}
  ...
  // 'use strict';
  // --globals : some var that are defined as global var
  console.log(xiaoshao);
  // --unused & --undef : if commentize line 15, function cal will not be used and line 16 will has an undefined var sum
 ...
 // means type coercion in JavaScript is not support
  ...
  function jieCheng(max){
    if(max === 0){
      return 1;
    }else{
      // return arguments.callee(max - 1);
      return jieCheng(max - 1);
    }
  }
  ...
  // xioashao has no idea about this case
  ...
  var cat = (function() { var name = 'Anton'; return name }());
  console.log(cat);
  ...
  // but all the function in orgArray is same because g is const 10 after executing for loop
  console.log(orgArray[0](2));
